...
def train(model: nn.Module, optimizer_g: optim.Optimizer, optimizer_d: optim.Optimizer,
    ...
    for epoch in range(start_epoch, total_epochs):
        ...
        for i, (t, elev, ref) in enumerate(train_loader):
            ...
            input_g_norm = torch.cat([masked_ref_norm, mask], dim=1)
            output_g_norm = model(input_g_norm)
            output_g_norm = masked_ref_norm[:, :1] + output_g_norm * (1 - mask[:, :1])
            real_input_gd = ref_norm[:, :1]
            fake_input_gd = output_g_norm.detach()
            if real_input_gd.size(2) - anchor <= args.azimuth_blockage_range[1]:
                anchor = real_input_gd.size(2) - args.azimuth_blockage_range[1] - 1
            real_input_ld = maskutils.crop(real_input_gd, 0, anchor, real_input_gd.size(2), 
                                           args.azimuth_blockage_range[1])
            fake_input_ld = maskutils.crop(fake_input_gd, 0, anchor, fake_input_gd.size(2), 
                                           args.azimuth_blockage_range[1])
            real_score = model.discriminator(real_input_gd, real_input_ld)
            fake_score = model.discriminator(fake_input_gd, fake_input_ld)
