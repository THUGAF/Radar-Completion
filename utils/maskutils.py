...
def gen_random_blockage_mask(ref: torch.Tensor, azimuth_blockage_range: List = [5, 10], 
                             random_seed: int = 0) -> Tuple[torch.Tensor, torch.Tensor, int, int]:
...
def gen_fixed_blockage_mask(ref: torch.Tensor, azimuth_start_point: int, anchor: int, 
                            blockage_len: int) -> Tuple[torch.Tensor, torch.Tensor, int, int]:
    mask = torch.ones_like(ref).type_as(ref).to(ref.device)
...
def direct_filling(ref: torch.Tensor, azimuth_start_point: int, anchor: int, blockage_len: int,
                   used_elevation_order: int = 1) -> torch.Tensor:
