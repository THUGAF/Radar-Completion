...
def gen_random_blockage_mask(tensor: torch.Tensor, azimuth_blockage_range: List = [5, 10], random_seed: int = 0):
...
def gen_fixed_blockage_mask(tensor: torch.Tensor, anchor: int, blockage_len: int):
    mask = torch.ones_like(tensor).type_as(tensor).to(tensor.device)
    mask[:, 0, anchor: anchor + blockage_len] = 0.0
    masked_tensor = tensor * mask
    return masked_tensor, mask, anchor, blockage_len